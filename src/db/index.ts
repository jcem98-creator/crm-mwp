...
export class PostgresMemoryDB {
    ...
    public async initialize() {
        if (!this.pool) {
            const connectionString = process.env.DATABASE_URL || 
                `postgresql://admin:tradamed@botlylatam-postgres-bskmfa:5432/db`;
            ...
            await this.pool.query(`
                CREATE TABLE IF NOT EXISTS leads_status (
                    ...
                    last_bot_message_at TIMESTAMPTZ DEFAULT NOW(),
            ...
            console.log("[DB] PostgreSQL conectado y tablas creadas.");
        }
    }
    ...
    public async updateLeadStatus(chatId: string | number, update: { 
        ...
        if (last_bot_at) {
            await this.pool!.query(`UPDATE leads_status SET last_bot_message_at = NOW() WHERE chat_id = $1`, [String(chatId)]);
        }
    ...
    public async getAllLeads(): Promise<any[]> {
        await this.initialize();
        const result = await this.pool!.query(`SELECT * FROM leads_status ORDER BY last_bot_message_at DESC`);
        return result.rows;
    }
