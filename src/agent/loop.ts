...
export async function runAgentLoop(chatId: string, initialMessage: string) {
    ...
    try {
        ...
        const hasExplicitBookingWords = msgNormalized.match(/(agendar|reservar|visitar|ir a su local|llamame|llamenme|cita|reunion|comunicarme|hablar con alguien|hablar con un asesor|apartar|separar|pago|deposito|anticipo|como les pago)/);
        ...
        const isEnglish = /(hi+|hello+|hey+|good\s+(morning|afternoon|evening)|how\s+are\s+you|price|cost|info|wedding|call|visit|what|tell|services|speak|english|simple|package)/i.test(msgNormalized) || extractedData.idioma === 'inglés' || extractedData.idioma === 'english';
